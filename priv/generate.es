...
gen_python(#xmlElement{name=node}=E, #state{indent=L, name=Name}=S) ->
    ...
    ["#!/usr/bin/env python\n",
     ...
     "import dbus.service\n",
     "\n",
     gen_python(Iface, S#state{indent=L+1}),
     "\n",
...
gen_python(#xmlElement{name=interface, content=Content}=E, #state{name=Name, indent=L}=S) ->
    [
     ...
     indent(L), "def __init__(self):\n",
     indent(L+1), "bus_name = dbus.service.BusName(SERVICE, bus=dbus.SessionBus())\n",
     indent(L+1), "dbus.service.Object.__init__(self, bus_name, '/')\n\n",
     [ gen_python(Method, S) || Method <- Content, is_record(Method, xmlElement), Method#xmlElement.name == method ],
     "\n",
     indent(L), "@dbus.service.method(dbus.PROPERTIES_IFACE, in_signature='ss', out_signature='v')\n",
     indent(L), "def Get(self, interface_name, property_name):\n",
     indent(L+1), "return \"\"\n\n",
     indent(L), "@dbus.service.method(dbus.PROPERTIES_IFACE, in_signature='s', out_signature='a{sv}')\n",
     indent(L), "def GetAll(self, interface_name):\n",
     indent(L+1), "return {}\n\n",
     indent(L), "@dbus.service.signal(dbus.PROPERTIES_IFACE, signature='sa{sv}as')\n",
     indent(L), "def PropertiesChanged(self, interface_name, property_name):\n",
     indent(L+1), "pass \n\n"     
    ];
gen_python(#xmlElement{name=method, content=C}=E, #state{indent=L}=S) ->
    InArgs = [ Arg || Arg <- C, 
